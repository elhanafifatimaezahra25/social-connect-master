...
pkgs.mkShell {
  ...
  shellHook = ''
    ...
    $MYSQL --protocol=TCP --host=127.0.0.1 --port=$PORT -u root -e "
      ...
      CREATE USER IF NOT EXISTS 'kamal'@'%' IDENTIFIED BY 'password';
      CREATE USER IF NOT EXISTS 'kamal'@'localhost' IDENTIFIED BY 'password';
      GRANT ALL PRIVILEGES ON testdb.* TO 'kamal'@'%';
      GRANT ALL PRIVILEGES ON testdb.* TO 'kamal'@'localhost';
      FLUSH PRIVILEGES;
    " >/dev/null 2>&1

    echo "📦 Database ready → testdb | user: kamal | pass: password"
    echo ""
    ...
    echo "  mysql connect:        mysql -h 127.0.0.1 -P $PORT -u kamal -p"
